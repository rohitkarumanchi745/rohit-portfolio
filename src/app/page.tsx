...
// Dynamically import 3D scene to avoid SSR issues - load after initial page render
...
const projects = [
  ...
  },
  {
    ...
    subtitle: "Rust + Python + FastAPI + GraphQL + REST + WebSocket + React Native + Kubernetes + FL/RL + PyTorch + OpenCV",
    points: [
      "Built production full-stack dating app: Rust backend with Python FastAPI microservices for GraphQL/REST APIs and WebSocket support, React Native/Expo mobile app, MCP server for customer service analytics.",
      ...
      "Utilized MCP (Model Context Protocol) server for customer service analytics, enabling real-time insights and automated response generation.",
      "Designed microservices with REST/GraphQL APIs, WebSocket real-time messaging, OTP/JWT auth, profile management, and production-grade error handling.",
      "System Design: Microservices architecture with Rust core backend and Python ML services, event-driven async messaging, multi-tier data layer (Postgres/Vector DB/Redis), distributed ML training pipeline with Federated Learning, GraphQL/REST API gateway for unified client access, WebSocket connections for real-time chat, MCP integration for customer service analytics, and comprehensive observability via Streamlit dashboards.",
    ],
    stack: ["Rust", "Python", "FastAPI", "GraphQL", "REST API", "WebSocket", "React Native/Expo", "PyTorch", "CNN", "OpenCV", "Vector DB", "Kubernetes", "Docker", "MCP Server", "FL/RL", "Airflow", "Ray/Anyscale", "Streamlit", "Data Fusion", "Postgres"],
      ...
      <div className="pointer-events-none fixed inset-0 z-0">
        ...
        <div className="absolute inset-0 will-change-transform">
          {[
            ...
            { icon: '🐘', size: 28, left: 18, top: 68, hideOnMobile: true, mobileSize: 16 },   // PostgreSQL
            { icon: '❄️', size: 26, left: 92, top: 35, hideOnMobile: true, mobileSize: 14 },   // Snowflake
            { icon: '🟧', size: 28, left: 56, top: 72, hideOnMobile: true, mobileSize: 16 },   // Databricks
            { icon: '🔄', size: 26, left: 88, top: 48, hideOnMobile: true, mobileSize: 14 },   // Kafka
            { icon: '📊', size: 28, left: 62, top: 18, hideOnMobile: true, mobileSize: 16 },   // Analytics
            { icon: '⚡', size: 26, left: 3, top: 42, hideOnMobile: true, mobileSize: 14 },    // Spark
            { icon: '🐳', size: 28, left: 92, top: 72, hideOnMobile: true, mobileSize: 16 },   // Docker
            ...
            return (
              <div
                key={`tech-${i}`}
                className={`absolute ${opacityClass} transition-all duration-300 ${item.hideOnMobile ? 'hidden md:block' : ''}`}
                style={{
                  fontSize: `${finalMobileSize}px`,
                  left: `${item.left}%`,
                  top: `${item.top}%`,
                  animation: `float ${duration}s ease-in-out infinite`,
                  animationDelay: `${delay}s`,
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 sm:px-6 pt-20 sm:pt-28 lg:pt-32 pb-12 sm:pb-20">
        ...
        <section id="about" className="relative">
          ...
          <div className="grid gap-8 sm:gap-12 lg:grid-cols-2 lg:gap-8 items-center">
            {/* Right: 3D Scene - Show first on mobile */}
