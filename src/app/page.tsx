...
const projects = [
  ...
  },
  {
    ...
    points: [
      "System Architecture: Architected a multi-layer backend in Rust using Axum, designed for horizontal scaling with stateless API servers and shared state via Redis. Implemented connection pooling (SQLx + PgBouncer patterns) and async I/O throughout using Tokio runtime to handle 10K+ concurrent connections per node.",
      "API Layer & Data Access: Built a GraphQL API with async-graphql featuring DataLoader for batched database queries (solving N+1), field-level authorization, and automatic schema introspection. Designed efficient data models with JSONB for flexible attributes, composite indexes for query optimization, and read replicas awareness.",
      "Real-Time Infrastructure: Designed a WebSocket pub/sub system with in-memory broadcast channels per chat room, supporting typing indicators, read receipts, and message fanout. Extended to WebRTC signaling for video/voice calls — managing session state, offer/answer exchange, ICE candidate relay, and graceful disconnection handling.",
      "ML Pipeline & Fraud Prevention: Integrated ONNX Runtime (tract) for low-latency inference — face detection, liveness verification, and embedding extraction. Built a similarity search layer using cosine distance on face embeddings to detect duplicate/fraudulent accounts. Designed for CPU-bound workloads with async offloading to prevent blocking.",
      "Matching Engine & Recommendations: Implemented a preference-based matching algorithm with compatibility scoring across multiple dimensions (interests, location, preferences). Added geo-filtering with distance calculations, bloom filters for seen-user deduplication, and configurable ranking weights for AI-assisted discovery.",
      "Monetization & Access Control: Designed tiered subscription system with feature gating via JWT claims — premium unlocks (unlimited swipes, read receipts, advanced filters). Implemented student verification flow with discount tiers and expiration handling. Role-based access control at resolver level.",
      "Observability & Reliability: Added Prometheus metrics for request latency histograms, active connections, cache hit rates, and error budgets. Implemented structured tracing with correlation IDs across services. Health endpoints with deep dependency checks (DB, Redis, ML models) for orchestrator probes.",
      "Mobile & Infrastructure: React Native/Expo mobile app with real-time sync. Kubernetes deployment with Docker multi-stage builds, CI/CD pipelines, and MCP server integration for customer service analytics.",
    ],
          ...
          <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl border border-slate-300 dark:border-white/10 bg-white/80 dark:bg-white/5 p-6 sm:p-8 lg:p-10 md:backdrop-blur-sm">
            ...
            <div className="relative">
              ...
              <ul className="mt-4 sm:mt-6 space-y-2 sm:space-y-3">
                {[
                  "Built real-time data platform: Kafka + Spark + Snowflake for retail analytics serving 100+ users",
                  "Designed AI Merchandising Copilot (FastAPI + LangGraph): Cut item setup time 65%, reduced errors from 14% to 3%",
                  "Delivered $1.5M savings in shipping operations, improved associate efficiency 250%",
                  "Built ML pipelines with Python/TensorFlow/SageMaker: $5M+ annual cost savings in supply chain optimization",
                ].map((item) => (
